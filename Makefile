# Makefile (Odin; OOS-build support; gmake-form/style; v2024.05.13)
...
BUILD_PATH := $()## path to build storage relative to makefile (defaults to 'build' [for `odin`]); used to create ${BUILD_DIR} which is then used as the base path for build outputs
...
CONFIG := release## default build configuration (debug/release)
...
# detect `odin`
ifeq (,$(shell odin version >${devnull} 2>&1 <${devnull} && echo `odin` present))
$(call %error,Missing required compiler (`odin`))
endif
...
# `odin` version determination
s := $(lastword $(shell odin version 2>${devnull}))
# $(call %debug_var,s)

# take first word as full version
ODIN_version := $(firstword ${s})

$(call %debug_var,ODIN_version)
...
BUILD_DIR := ${BASEPATH}/$(or ${BUILD_PATH},build)## note: `${HASH}build` is not a 'valid' target path for `odin`
...
BIN_DIR := $(firstword $(wildcard $(foreach src_path,${SOURCE_dirs} .,$(foreach segment,bin bins,${src_path}/${segment}))))
BIN_DIR_filename := $(notdir ${BIN_DIR})
BIN_OUT_DIR_bin := ${OUT_DIR_bin}/${BIN_DIR_filename}
...
BIN_SRC_packages := $(and ${BIN_DIR},$(patsubst %/,%, $(dir $(wildcard $(foreach elem,${SOURCE_exts},${BIN_DIR}/${elem})))))
...
BIN_bin_files := $(call %BIN_bin_of_src,${BIN_SRC_packages})
...
$(call %debug_var,BIN_SRC_packages)
...
.DEFAULT_GOAL := $(if ${SRC_files},${PROJECT_TARGET},$(if ${BIN_SRC_packages},bins,$(if ${EG_SRC_packages},examples,)))# *default* target
...
ifeq (${NULL},${ODIN_version})
msg := `odin` not found (`odin version` unrecognizable)
ifeq (${OSID},win)
msg := ${msg}; try installing `odin` via Scoop (`scoop install odin`)
else
msg := ${msg}; see <https://odin-lang.org/docs/install> for installation instructions
endif
$(call %warning,${msg})
endif
